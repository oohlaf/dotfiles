...
sub nickserv_notice {
    ...
    if (is_nickserv($server->{tag}, $address)) {
        if ($text =~ /^If this is your nickname, type \/msg NickServ/ || $text =~ /^Please identify via/ || $text =~ /^This nickname is registered and protected.  If it is your/ || $text =~ /This nickname is registered\. Please choose a different nickname,/ || $text =~ /^This nickname is registered. Please choose a different nickname/) {
